...
public class MergeJoinPlan implements Plan {
    ...
    public MergeJoinPlan(Plan p1, Plan p2, String fldname1, String fldname2, Transaction tx) {
        ...
        // CS4432
        // When a MergeJoinPlan is constructed, check to see if the the tables
        // of the given plans have already been sorted. If already sorted,
        // do not construct SortPlans for those plans, and instead simply
        // use the unaltered plan as passed in.
        TablePlan tp1 = (TablePlan) p1;
        TablePlan tp2 = (TablePlan) p2;

        // Only create sort plans for the two queries if the
        // tables are not sorted
        if(!tp1.getTi().isSorted()) {
            this.p1 = new SortPlan(p1, sortlist1, tx);
        }
        else {
            // Otherwise, the query plans should stay unchanged
            this.p1 = p1;
        }

        if(!tp2.getTi().isSorted()) {
     ...
     */
    public Scan open() {
        // If the plans are already sorted then the following
        // scans will not re-sort the plans as the plans will
        // not be Sort Plans.
        Scan s1 = p1.open();
